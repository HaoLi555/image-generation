...
class Decoder(nn.Module):
    def __init__(self, channel, in_dim) -> None:
        ...
        self.convtrans_block=nn.Sequential(
            nn.ConvTranspose(256, 256, kernel_size=4, stride=2, padding=1), # 256, 8, 8
            nn.ReLU(),
            nn.BatchNorm(256),
            nn.ConvTranspose(256, 256, kernel_size=3, stride=1, padding=1), # 256, 8, 8
            nn.ReLU(),
            nn.BatchNorm(256),
             nn.ConvTranspose(256, 128, kernel_size=4, stride=2, padding=1), # 128, 16, 16
            nn.ReLU(),
            nn.BatchNorm(128),
            nn.ConvTranspose(128, 128, kernel_size=3, stride=1, padding=1), # 128, 16, 16
            nn.ReLU(),
            nn.BatchNorm(128),
            nn.ConvTranspose(128, 64, kernel_size=4, stride=2, padding=1), # 64, 32, 32
            nn.ReLU(),
            nn.BatchNorm(64),
            nn.ConvTranspose(64, 3, kernel_size=3, stride=1, padding=1), # 3, 32, 32
        )
...
class Beta_VAE_Manager():
    def __init__(self, train_loader, test_loader, wandb_run, num_epochs=50, show_real=True, beta=2) -> None:
        ...
        self.optimizer = nn.Adam(self.model.parameters(), lr=1e-4, betas=(0.9, 0.999))
